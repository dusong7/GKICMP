...
{
    public partial class AssetsFilesList : PageBase
    {
        ...
        private void DataBindList()
        {
            ...
            //DataTable dt = project_FileDAL.GetPaged(6, 1, ref recordCount, model);
        ...
        protected void btn_Delete_Click(object sender, EventArgs e)
        {
            ...
            //    {
            //        ShowMessage("删除成功");
            //        sysLogDAL.Edit(new SysLogEntity((int)CommonEnum.LogType.操作日志, "删除资产文件", UserID));
            //    }
            //    else
            //    {
            //        ShowMessage("删除失败");
        ...
        protected void lbtn_SB_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                string id = lbtn.CommandArgument.ToString();  //PID
                ...
                //根据PID循环依次上传多个附件（适用多个附件数据库多条记录，即每个附件对应数据集的一条数据）
                DataTable bt = project_FileDAL.GetList(id);
                for (int d = 0; d < bt.Rows.Count; d++)
                {
                    if (bt.Rows[d]["FileUrl"]  != null)
                    {
        ...
        protected void lbtn_MoreSB_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < id.Split(',').Length; i++)
                {
                    DataTable bt = project_FileDAL.GetList(id.Split(',')[i]);
