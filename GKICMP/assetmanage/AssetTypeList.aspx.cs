...
{
    public partial class AssetTypeList : PageBase
    {
        ...
        private string GetTableContent()
        {
            ...
            DataTable dtFirst = assetTypeDAL.GetType(-1, (int)CommonEnum.Deleted.未删除, 1);
 ...
 font-size: 14px;' onclick='jQuery(&#39;#example-advanced&#39;).treetable(&#39;collapseAll&#39;); return false;'>&nbsp;全部收起&nbsp;</a>");
        ...
        {
            StringBuilder sb = new StringBuilder("");
            DataTable dv = assetTypeDAL.GetType(parentid, (int)CommonEnum.Deleted.未删除, 1);
            if (dv.Rows.Count > 0)
            {
                for (int i = 0; i < dv.Rows.Count; i++)
                {
                    ...
                    sb.Append("<td><span class='treebg'>");
                    sb.Append("<b1></b1>");
                    sb.Append("</span><a href='#' onclick='return showbox(" + dv.Rows[i]["SDID"].ToString() + ")'><span class='file'>" + dv.Rows[i]["DataName"].ToString() + "</span></a></td>");
        ...
        protected void btn_Delete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                int result = assetTypeDAL.DeleteBat(lsid, (int)CommonEnum.Deleted.删除);
                if (result > 0)
                {
                    ShowMessage("删除成功");
