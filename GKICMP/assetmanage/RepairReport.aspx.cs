...
{
    public partial class RepairReport : PageBase
    {
        ...
        protected void btn_OutPut_Click(object sender, EventArgs e)
        {
            ...
            str.Append(@"<table border='1' cellpadding='0' cellspacing='0' >
                                     <tr>
                                        ...
                                        <th><strong>未处理</strong></th>
                                        <th><strong>已受理</strong></th>
                                        <th><strong>已完成</strong></th>
                                        <th><strong>确认完成</strong></th>
                                     </tr>");
            if (dt != null && dt.Rows.Count > 0)
            {
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    str.AppendFormat("<td>{0}</td>", row["wcl"]);
                    str.AppendFormat("<td>{0}</td>", row["sl"]);
                    str.AppendFormat("<td>{0}</td>", row["wc"]);
                    str.AppendFormat("<td>{0}</td>", row["qrwc"]);
                    str.Append("</tr>");
                }
            }
            CommonFunction.ExportExcel("报修记录教师统计", str.ToString());
