...
{
    public partial class MRoomWeekView : PageBase
    {
        ...
        private void BindRoom(DateTime startweek)
        {
            ...
            DataView dvroom = meetDAL.GetApplyList(Convert.ToDateTime(begindate), Convert.ToDateTime(enddate), (int)CommonEnum.Deleted.未删除);
        ...
        /// <returns></returns>
        private static string BindDay(DateTime startweek, DataView dvroom, DataRow row, int type)
        {
            int ischecked = 0;
            ...
            for (int i = 0; i < 7; i++)
            {
                ...
                {
                    //DateTime mbegin = Convert.ToDateTime(rrow["MBegin"]);
                    //DateTime mend = Convert.ToDateTime(rrow["MEnd"]);
                    //string mmbegin = mbegin.ToString("yyyy-MM-dd");
                    //string mmend = mend.ToString("yyyy-MM-dd");
                    //if ((Convert.ToDateTime(mmend) - Convert.ToDateTime(mmbegin)).Days > 0 && type == 1 && begin == mmbegin)
                    //{
                    //    mend = Convert.ToDateTime(mmbegin + " 12:00");
                    //}
                    //if ((Convert.ToDateTime(mmend) - Convert.ToDateTime(mmbegin)).Days > 0 && type == 2 && begin == mmbegin)
                    //{
                    //    mbegin = Convert.ToDateTime(mmbegin + " 12:01");
                    //    mend = Convert.ToDateTime(mmbegin + " 23:59");
                    //}
                    //if ((Convert.ToDateTime(mmend) - Convert.ToDateTime(mmbegin)).Days > 0 && type == 1 && begin == mmend)
                    //{
                    ...
                    //    }
                    //}
                    //if ((Convert.ToDateTime(mmend) - Convert.ToDateTime(mmbegin)).Days > 0 && type == 2 && begin == mmend)
                    //{
                    ...
                    //if ((mbegin >= Convert.ToDateTime(beginday) && mbegin <= Convert.ToDateTime(endday)) && mbegin.ToString("yyyy-MM-dd") == begin && row["CRID"].ToString() == rrow["MeetingRoom"].ToString() && type == 1)
                    //{
                    //    ischecked = 1;
                    //    title += "\"" + rrow["MTitle"] + "【" + mbegin.ToString("HH:mm") + "-" + mend.ToString("HH:mm") + "】\"&#10";
                    //    // break;
                    //}
                    //if ((mend >= Convert.ToDateTime(beginday) && mend <= Convert.ToDateTime(endday)) && row["CRID"].ToString() == rrow["MeetingRoom"].ToString() && type == 2)
                    //{
                    //    ischecked = 1;
                    //    title += "\"" + rrow["MTitle"] + "【" + mbegin.ToString("HH:mm") + "-" + mend.ToString("HH:mm") + "】\"&#10";
                    //    //break;
                    ...
                    if ((mbegin >= Convert.ToDateTime(beginday) && mbegin <= Convert.ToDateTime(endday)) &&  row["CRID"].ToString() == rrow["MeetingRoom"].ToString() && type == 1)
                    {
                        ischecked = 1;
                        title += "\"" + rrow["MTitle"] + "【" + mbegin.ToString("HH:mm") + "-" + mend.ToString("HH:mm") + "】\"&#10";
                        // break;
                ...
                {
                    sbrooms.AppendFormat(" <td style='background: #ACD6FF;text-align:center; cursor:pointer;' title='{1}'>{0}</td>", name, title);
                }
                ischecked = 0;
            }
