...
{
    public partial class FileLogManage : PageBase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ViewState["CreateUser"] = this.txt_CreateUser.Text;
        ...
        {
            int recordCount = -1;
            SysLogEntity model = new SysLogEntity();
            model.CreateUser = ViewState["CreateUser"].ToString();
            model.LogType = 0;
            model.LogFlag = (int)CommonEnum.LogFlag.文件日志;
