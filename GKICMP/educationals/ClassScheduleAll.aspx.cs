...
{
    public partial class ClassScheduleAll : PageBase
    {
        ...
        public static string oldEYear = "";
        public static int oldterm = 0;
        ...
        private void Ddl()
        {
            this.ddl_EYear.Items.Clear();
            this.ddl_Term.Items.Clear();
        ...
        {
            string sql3 = "and Isdel=0 and EYear='" + this.ddl_EYear.SelectedValue + "' and Term=" + this.ddl_Term.SelectedValue + " order by Position";
            DataTable dt3 = scourseDAL.GetAllScheduleCourseByWhere(sql3);
            if (dt3 != null && dt3.Rows.Count > 0)
            {
                this.btn_Quote.Visible = false;
            }
            else
            {
                if (term == (int)CommonEnum.XQ.上学期)
                {
                    oldEYear = (Convert.ToInt32(EYear.Substring(0, 4).ToString()) - 1).ToString() + "-" + EYear.Substring(0, 4).ToString();
                    oldterm = (int)CommonEnum.XQ.下学期;
                }
                else
                {
                    oldEYear = EYear;
                    oldterm = (int)CommonEnum.XQ.上学期;
                }
                DataTable dt2 = scourseDAL.GetClaID(EYear, term);
                if ((dt2 == null || dt2.Rows.Count == 0) && EYear == this.ddl_EYear.SelectedValue && term == Convert.ToInt32(this.ddl_Term.SelectedValue))
                {
                    this.btn_Quote.Visible = true;
                }
                else
                {
                    this.btn_Quote.Visible = false;
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                str.Append("<table border='0' cellpadding='0' cellspacing='0' id='textTable' width='100%' class='content table table-bordered scrollTable' >");
                str.Append("<thead><tr>");
                ...
                for (int k = 0; k < dt.Rows.Count; k++)
                {
                    ...
                    {
                        ShowMessage("暂无数据");
                        return " <div class=\"datanone\">抱歉！暂无课表数据</div>"; ;
                    }
                    ...
                    for (int c = 1; c <= WeekDays; c++)
                    {
                        for (int a = 1; a <= WSnum + SWnum + XWnum; a++)
                        {
                            ...
                            else
                            {
                                ...
                                //if (aa != "" && aa != "无课")
                                //{
                                //    str.AppendFormat("<td class='contd2' title='" + title + "'>{0}</td>", aa);
                                //}
                            }
                        }
                    }
        ...
        protected void btn_Quote_Click(object sender, EventArgs e)
        {
            try
            {
                int result = scourseDAL.YYSXQKB(oldEYear, oldterm, EYear, term);
                if (result == 0)
                {
                    sysLogDAL.Edit(new SysLogEntity((int)CommonEnum.LogType.操作日志_其他, "引用" + oldEYear + "学年度" + CommonFunction.CheckEnum<CommonEnum.XQ>(oldterm) + "课表信息", UserID));
                    ShowMessage("引用成功");
                    Ddl();
                    this.lbl.Text = DataBindList();
                }
