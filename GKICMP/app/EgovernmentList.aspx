<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="EgovernmentList.aspx.cs" Inherits="GKICMP.app.EgovernmentList" %>
    ...
    <form id="form1" runat="server">
        ...
        <div class="mui-content">
            <div style="padding: 10px 10px;">
                <div id="segmentedControl" class="mui-segmented-control">
                    ...
                    <a href="EgovernmentList.aspx" class="mui-control-item mui-active">已收政务</a>
                </div>
            </div>
        ...
        <script>
            ...
            $.ajax({
                ...
                cache: false,
                type: "GET",
                data: "method=Egovernment&pageindex=1&pagesize=10&didname=" + $("#hf_didname").val(),
                dataType: "json",
                async: false,
                    ...
                    if (data.result == "success") {
                        for (var j in data.data) {
                            ...
                            intem += " <li class='mui-table-view-cell'><a href='EgovernmentDetails.aspx?id=" + data.data[j].FID + "'> <div class='mui-table'><div class='mui-col-xs-12'><h4 class='mui-ellipsis'> "
                                ...
                                + "</span></h5></div></div></a> ";
                            if (data.data[j].Completed == 0 && data.data[j].State == 5 && data.data[j].IsApproved==1) {
                                intem += " <div class='list-btn' ><a href='EgovermentPZEdit.aspx?id=" + data.data[j].FID + "'  class='btn-pz'>批转</a> </div>";
                            }
                            intem += "</li>";
            ...
            function pullupRefresh() {
                setTimeout(function () {
                    ...
                    $.ajax({
                        ...
                        cache: false,
                        type: "GET",
                        data: "method=Egovernment&pagesize=10&pageindex=" + i + "&didname=" + $("#hf_didname").val(),
...
<script>
    function getdata(name) {
        $.ajax({
            ...
            cache: false,
            type: "GET",
            data: "method=Egovernment&pageindex=1&pagesize=10&didname=" + name,
            dataType: "json",
            async: false,
            success: function (data) {
                if (data.result == "success") {
                    for (var j in data.data) {
                        ...
                        intem += " <li class='mui-table-view-cell'><a href='EgovernmentDetails.aspx?id=" + data.data[j].FID + "'> <div class='mui-table'><div class='mui-col-xs-12'><h4 class='mui-ellipsis'> "
                                 ...
                                 + (data.data[j].Completed == "1" ? "<span style='color:#FF83FA'>已归档</span>" : data.data[j].State == "0" ? "<span style='color:red'>未处理</span>" : data.data[j].State == "1" ? "<span style='color:blue'>批转中</span>" : data.data[j].State == "2" ? "已处理" : data.data[j].State == "5" ? "<span style='color:#48bd81'>已阅</span>" : "")
                                 + "</span></h5></div></div></a> ";
                        if (data.data[j].Completed == 0 && data.data[j].State == 5) {
                            intem += " <div class='list-btn' ><a href='EgovermentPZEdit.aspx?id=" + data.data[j].FID + "'  class='btn-pz'>批转</a> </div>";
                        }
                        intem += "</li>";
                        $("#pullrefresh").append(intem);
