<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="LeaveRecordList.aspx.cs" Inherits="GKICMP.app.LeaveRecordList" %>
    ...
    <form id="form1" runat="server">
        ...
        <div class="mui-content">
            <div style="padding: 10px 10px;">
                <div id="segmentedControl" class="mui-segmented-control">
                      <a href="LeaveRecord.aspx" class="mui-control-item ">备案登记</a>
                      <a href="LeaveRecordList.aspx" class="mui-control-item mui-active">我发起的</a>
                </div>
            </div>
        ...
        <script>
            ...
            $.ajax({
                ...
                cache: false,
                type: "GET",
                data: "method=PeopleRecord&pageindex=1&pagesize=10",
                dataType: "json",
                async: false,
                success: function (data) {
                    if (data.result == 'true') {
                        for (var j in data.data) {
                            ...
                            intem += "<a href='leaveRecordDetail.aspx?id=" + data.data[j].LID + "&laid='> ";
                            intem += "<div class='mui-table'>";
                            intem += "<div class='mui-col-xs-12'>";
                            intem += "<h4 class='mui-ellipsis'>";
                            intem += "<span style='margin-left: -6px;'>" + "【" + data.data[j].BeginDate + "至" + data.data[j].EndDate + "】</span>";

                            intem += "</h4>";
                            intem += "</div>";
                            intem += "<div class='mui-col-xs-12'>";
                            intem += "<h5 class='mui-ellipsis'>";
                            intem += "天数：" + data.data[j].LeaveDays + "&nbsp;&nbsp;事由：" + data.data[j].LeaveMark + "天";
                            intem += "</h5>";
                            intem += "</div>";
                            intem += "</div>";
                            intem += "</a>";
                                intem += " <div class='list-btn' ><span style='margin-right: 10px;padding: 5px 10px;border-radius: 15px;' id='" + data.data[j].LID + "' class='btn-pz'>撤销</span></div>";
            ...
            function pullupRefresh() {
                setTimeout(function () {
                    ...
                    $.ajax({
                        ...
                        cache: false,
                        type: "GET",
                        data: "method=PeopleRecord&pagesize=10&pageindex=" + i,
                        ...
                        success: function (data) {
                            if (data.result == 'true') {
                                for (var j in data.data) {
                                    var intem = "";
                                    intem += "<li class='mui-table-view-cell'>";
                                    intem += "<a href='LeaveDetail.aspx?id=" + data.data[j].LID + "&laid='> ";
                                    intem += "<div class='mui-table'>";
                                    intem += "<div class='mui-col-xs-12'>";
                                    intem += "<h4 class='mui-ellipsis'>";
                                    intem += "<span style='margin-left: -6px;'>" + "【" + data.data[j].BeginDate + "至" + data.data[j].EndDate +"】</span>";
