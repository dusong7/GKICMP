<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="OverTimeAudit.aspx.cs" Inherits="GKICMP.app.OverTimeAudit" %>
    ...
    <form id="form1" runat="server">
        ...
        <div class="mui-content">
            <div style="padding: 10px 10px;">
                <div id="segmentedControl" class="mui-segmented-control">
                     <a href="OverTimeEdit.aspx" class="mui-control-item">加班申请</a>
                <a href="OverTime.aspx" class="mui-control-item">我发起的</a>
                <a href="OverTimeAudit.aspx" class="mui-control-item mui-active">我审批的</a>
                </div>
            </div>
        ...
        <script>
            ...
            $.ajax({
                ...
                cache: false,
                type: "GET",
                data: "method=OverTimeAudit&pageindex=1&pagesize=10",
                dataType: "json",
                async: false,
                success: function (data) {
                    if (data.result == 'true') {
                        for (var j in data.data) {
                            ...
                            intem += "<a href='OverTimeDetail.aspx?id=" + data.data[j].LID + "+&laid=" + data.data[j].LAID + "'> ";
                            ...
                            intem += "日期：" + data.data[j].BeginDate + "至" + data.data[j].EndDate;
                            intem += "</h5>";
                            intem += "<h5 class='mui-ellipsis'>";
                            intem += "天数：" + data.data[j].LeaveDays + "天";
                            intem += "</h5>";
                            intem += "<h5 class='mui-ellipsis'>参与人：" + data.data[j].UsersName + "</h5>";
                            intem += "</div>";
                            intem += "</div>";
                            intem += "</a>";
                            if (data.data[j].LeaveStateID != 2 && data.data[j].LeaveStateID != 3) {
                                intem += " <div class='list-btn' ><a href='OverTimeAuditEdit.aspx?&flag=1&id=" + data.data[j].LID + "&laid=" + data.data[j].LAID + "'  class='btn-pz'>审核</a> </div>";
                            }
                            intem += "</li>";
            ...
            function pullupRefresh() {
                setTimeout(function () {
                    ...
                    $.ajax({
                        ...
                        cache: false,
                        type: "GET",
                        data: "method=OverTimeAudit&pagesize=10&pageindex=" + i,
