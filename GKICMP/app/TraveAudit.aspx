<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="TraveAudit.aspx.cs" Inherits="GKICMP.app.TraveAudit" %>
    ...
    <form id="form1" runat="server">
        ...
        <div class="mui-content">
            <div style="padding: 10px 10px;">
                <div id="segmentedControl" class="mui-segmented-control">
                    ...
                    <a href="TraveAudit.aspx" class="mui-control-item mui-active">我审批的</a>
                </div>
            </div>
        ...
        <script>
            ...
            $.ajax({
                ...
                cache: false,
                type: "GET",
                data: "method=TraveAudit&pageindex=1&pagesize=10",
                dataType: "json",
                async: false,
                success: function (data) {
                    if (data.result == 'true') {
                        for (var j in data.data) {
                            ...
                            intem += "<a href='TraveDetail.aspx?id=" + data.data[j].LID + "+&laid=" + data.data[j].LAID + "'> ";
                            ...
                            intem += "日期：" + data.data[j].BeginDate + "至" + data.data[j].EndDate + "&nbsp;&nbsp;天数：" + data.data[j].LeaveDays + "天"; intem += "</h5>";
                            intem += "</div>";
                            intem += "</div>";
                            intem += "</a>";
                            if (data.data[j].LeaveStateID != 2 && data.data[j].LeaveStateID != 3) {
                                intem += " <div class='list-btn' ><a href='LeaveAuditEdit.aspx?&flag=2&id=" + data.data[j].LID + "&laid=" + data.data[j].LAID + "'  class='btn-pz'>审核</a> </div>";
                            }
                            intem += "</li>";
            ...
            function pullupRefresh() {
                setTimeout(function () {
                    ...
                    $.ajax({
                        ...
                        cache: false,
                        type: "GET",
                        data: "method=TraveAudit&pagesize=10&pageindex=" + i,
                        ...
                        success: function (data) {
                            if (data.result == 'true') {
                                for (var j in data.data) {
                                    ...
                                    intem += "<div class='mui-col-xs-12'>";
                                    intem += "<h4 class='mui-ellipsis'>";
                                    intem += data.data[j].LeaveUserName;
                                    intem += "</h4>";
                                    intem += "</div>";
                                    intem += "<div class='mui-col-xs-12'>";
                                    intem += "<h5 class='mui-ellipsis'>";
                                    intem += "外出天数：" + data.data[j].LeaveDays + "天" + "&nbsp;&nbsp;状态：" + "<span style='color:" + (data.data[j].LeaveStateID == "1" ? "red" : "green") + "'>" + data.data[j].LeaveState + "</span>";
                                    intem += "</h5>";
                                    intem += "</div>";
                                    intem += "<div class='mui-col-xs-12'>";
                                    intem += "<h5 class='mui-ellipsis'>";
                                    intem += "开始日期：" + data.data[j].BeginDate + "&nbsp;&nbsp;结束日期：" + data.data[j].EndDate;
                                    intem += "</h5>";
                                    intem += "</div>";
                                    intem += "</div>";
                                    intem += "</a>";
