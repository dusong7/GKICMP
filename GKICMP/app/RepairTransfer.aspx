<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="RepairTransfer.aspx.cs" Inherits="GKICMP.app.RepairTransfer" %>
    ...
    <form id="form1" runat="server">
        <header class="mui-bar mui-bar-nav">
          <%--  <a class="mui-action-back mui-icon mui-icon-left-nav mui-pull-left"></a>--%>
            <h1 class="mui-title">维修管理</h1>
        </header>
        <div class="mui-content">
           <%-- <div style="padding: 10px 10px;">
                <div id="segmentedControl" class="mui-segmented-control">
                    <a href="RepairTransfer.aspx" class="mui-control-item">待完成</a>
                    <a href="RepairList.aspx" class="mui-control-item mui-active">我受理的</a>
                </div>
            </div>--%>
            <div class=" mui-scroll-wrapper" id="offCanvasContentScroll" style="margin-top: 48px; margin-bottom: 50px">
        ...
        <script>
            ...
            $.ajax({
                ...
                cache: false,
                type: "GET",
                data: "method=RepairYJ&pageindex=1&pagesize=10",
                dataType: "json",
                async: false,
                success: function (data) {
                    if (data.result == 'true') {
                        for (var j in data.data) {
                            ...
                            intem += "</div>";
                            intem += "</a>";
                            if (data.data[j].ARState == -3) {
                                intem += "<div class='list-btn' > <a href='PeopleRepairEdit.aspx?id=" + data.data[j].ARID + "&flag=3'  class='btn-pz'>完成</a></div>";
                            }
                            //}
            ...
            function pullupRefresh() {
                setTimeout(function () {
                    ...
                    $.ajax({
                        ...
                        cache: false,
                        type: "GET",
                        data: "method=RepairYJ&pagesize=10&pageindex=" + i,
                        dataType: "json",
                        async: false,
                        success: function (data) {
                            if (data.result == 'true') {
                                for (var j in data.data) {
                                    ...
                                    intem += "</a>";
                                    if (data.data[j].ARState == -3) {
                                        intem += "<div class='list-btn' > <a href='PeopleRepairEdit.aspx?id=" + data.data[j].ARID + "'  class='btn-pz'>完成</a></div>";
                                    }
                                    ...
                                    //       + data.data[j].ARID + "' state='" + data.data[j].ARState + "' class='btn-sh'>受理</span></div>";
                                    //}
                                    //if (data.data[j].ARState == 1) {
                                    //    intem += " <div class='list-btn' ><a style='margin-right: 10px;padding: 5px 10px;border-radius: 15px;' id='"
                                    //         + data.data[j].ARID + "' state='" + data.data[j].ARState + "' class='btn-sh'>移交</a>";
