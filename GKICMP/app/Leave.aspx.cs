...
{
    public partial class Leave : PageBaseApp
    {
        ...
        protected void btn_Sumbit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (this.hf_LType.Value == "")
                {
                    ShowMessage("请选择请假类型");
                    return;
                }
                model.LType = int.Parse(this.hf_LType.Value);
                ...
                if (result == 0)
                {
                    ...
                    sysLogDAL.Edit(new SysLogEntity((int)CommonEnum.LogType.操作日志_添加, "添加请假信息", UserID));
                    //ShowMessage("提交成功");
                    RegisterStartupScript("false", "<script>alert('提交成功');window.location.href='PepoleLeave.aspx'</script>");
                }
                else if (result == -2)
                {
        ...
        protected void btn_Search_Click(object sender, EventArgs e)
        {
            DataBindList();
            GetAuditUser();
            this.lbl_LType.Text = "请假类型";
            this.lbl_LeaveDays.Text = "请假天数";
            this.lbl_sh.Text = "请假审核人";
        }
        #endregion
