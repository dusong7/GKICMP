...
{
    public partial class NetworkTeachEdit : PageBase
    {
        ...
        {
            //checkboxlist 绑定
            DataTable cla = departmentDAL.GetByShortName(int.Parse(this.ddl_EPID.SelectedValue), (int)CommonEnum.Deleted.未删除);
        ...
        public void InfoBind()
        {
            ...
            this.hf_UpImage.Value = model.ImgUrl;
            //string cla = "";
            //foreach (ListItem li in this.cbl_Class.Items) 
            //{
            //    if (li.Selected)
        ...
        protected void btn_Sumbit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (accessinfo.AccessID == "-2")
                {
                    ...
                    CommonFunction.delfile(hf_UpImage.Value.ToString());
                ...
                else
                {
                    if (this.fl_UpFile.HasFile)
                    {
                        ...
                        //抓取缩略图
                        string a = VideoThumbnail.ConvertImage(accessinfo.AccessUrl, "/webupload/ThumbImage", accessinfo.AccessName + ".png", "90*109");
                        model.ImgUrl = a == "" ? "\\webupload\\ThumbImage\\NoPic.png" : a.Replace("/","\\");
                    }
                    else
                    {
                        model.NTTUrl = this.hf_UpFile.Value;
                        model.ImgUrl = this.hf_UpImage.Value;
                    }
                }
                ...
                if (result > 0)
                {
                    sysLogDAL.Edit(new SysLogEntity((int)CommonEnum.LogType.系统日志, (NTID == "" ? "添加" : "修改") + "添加网路课程【" + this.txt_NTTName.Text + "】", UserID));
                    ShowMessage();
                }
                else
                {
