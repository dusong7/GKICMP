...
{
    public partial class NetworkTeachManageStu : PageBase
    {
        ...
        private void DataBindList()
        {
           ...
           // model.EPID = this.ddl_EPID.SelectedValue;
            DataTable dt = networkTeachDAL.GetPagedByStu(Pager.PageSize, Pager.CurrentPageIndex, ref recordCount, model);
        ...
        protected void lbtn_WatchOnline_Click(object sender, EventArgs e)
        {
            LinkButton lbtn = (LinkButton)sender;
            string id = lbtn.CommandArgument;
            string[] roomid = lbtn.CommandName.Split(',');
            if (roomid.Length > 1)
                Response.Redirect("TeacChat.aspx?id=" + id + "&roomid=" + roomid[0] + "&type=" + roomid[1], false);
            else
                Response.Redirect("TeacChat.aspx?id=" + id + "&roomid=" + roomid[0] + "&type=1", false);
        }
    }
}