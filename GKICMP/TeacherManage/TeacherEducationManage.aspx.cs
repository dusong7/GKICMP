...
{
    public partial class TeacherEducationManage : PageBase
    {
        ...
        {
            ViewState["EMajor"] = CommonFunction.GetCommoneString(this.txt_EMajor.Text.ToString().Trim());
        ...
        private void DataBindList()
        {
            ...
            model.EMajor = (string)ViewState["EMajor"].ToString();
            model.Begin = Convert.ToDateTime(ViewState["begin"]);
            model.End = Convert.ToDateTime(ViewState["end"]);
            // model.Education = Convert.ToInt32(ViewState["Education"].ToString());
            model.Isdel = (int)CommonEnum.Deleted.未删除;
            DataTable dt = teacherEducation.GetPaged(Pager.PageSize, Pager.CurrentPageIndex, ref recordCount, model, (string)ViewState["RealName"]);
        ...
        protected void btn_Delete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                int result = teacherEducation.DeleteBat(ids, (int)CommonEnum.Deleted.删除);
                if (result > 0)
                {
                    ShowMessage("删除成功");
