...
{
    public partial class TeacherGuidanceManage : PageBase
    {
        ...
        public void DataBindList()
        {
           ...
           // model.RGrade = ViewState["RGrade"].ToString();
            ...
            DataTable dt = teacher_GuidanceDAL.GetPaged(Pager.PageSize, Pager.CurrentPageIndex, ref recordCount, model, (string)ViewState["RealName"]);
        ...
        protected void btn_Delete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                int result = teacher_GuidanceDAL.DeleteBat(ids, (int)CommonEnum.Deleted.删除);
                if (result > 0)
                {
        ...
        protected void lbtn_SB_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    Teacher_GuidanceEntity dr = teacher_GuidanceDAL.GetObjByID(id.Split(',')[i]);
                ...
                {
                    int rusult = teacher_GuidanceDAL.Update(id);//更新字段为 已上报
        ...
        protected void btn_OutPut_Click(object sender, EventArgs e)
        {
            ...
            model.End = Convert.ToDateTime(ViewState["end"].ToString());
            model.Isdel = (int)CommonEnum.IsorNot.否;
            DataTable dt = teacher_GuidanceDAL.GetPaged(2000, 1, ref recordCount, model, (string)ViewState["RealName"]);
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    str.AppendFormat("<td>{0}</td>", row["RGrade"]);
                    str.AppendFormat("<td>{0}</td>", CommonFunction.CheckEnum<CommonEnum.URole>(row["GRole"].ToString()));
                    str.AppendFormat("<td>{0}</td>", row["PubDate"].ToString() == "" ? "" : Convert.ToDateTime(row["PubDate"]).ToString("yyyy-MM-dd"));

                    str.Append("</tr>");
                }
            }
            CommonFunction.ExportExcel("指导学生获奖导出", str.ToString());
