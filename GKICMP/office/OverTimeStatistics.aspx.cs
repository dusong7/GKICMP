...
{
    public partial class OverTimeStatistics : PageBase
    {
        ...
        {
           // int recordCount = 0;
            //DataTable dt = overTimeDAL.GetStatistcs(Pager.PageSize,Pager.CurrentPageIndex,ref recordCount, Convert.ToDateTime(ViewState["BeginDate"].ToString()), Convert.ToDateTime(ViewState["EndDate"].ToString()),int.Parse(this.ddl_OState.SelectedValue));
            DataTable dt = overTimeDAL.GetStatistcs(Convert.ToDateTime(ViewState["BeginDate"].ToString()), Convert.ToDateTime(ViewState["EndDate"].ToString()), int.Parse(this.ddl_OState.SelectedValue));
            ...
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    foreach (DataRow dr in dt.Rows)
                    {
                        ...
                        list.Add(dr["BeginDate"].ToString() == "" ? "" : Convert.ToDateTime(dr["BeginDate"]).ToString("yyyy-MM-dd HH:mm"));
                        list.Add(dr["EndDate"].ToString() == "" ? "" : Convert.ToDateTime(dr["EndDate"]).ToString("yyyy-MM-dd HH:mm"));
