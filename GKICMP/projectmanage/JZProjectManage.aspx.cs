...
{
    public partial class JZProjectManage : PageBase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                this.txt_Begin.Text = DateTime.Now.Year.ToString() + "-01-01";
                GetCondition();
                DataBindList();
            }
        }
        ...
        private void GetCondition()
        {
            ...
            ViewState["begin"] = this.txt_Begin.Text == "" ? DateTime.Now.Year.ToString()+"-01-01" : this.txt_Begin.Text;  //开始时间
        ...
        private void DataBindList()
        {
            ...
            DataTable dt = jzprojectManageDAL.GetPaged(Pager.PageSize, Pager.CurrentPageIndex, ref recordCount, model, Convert.ToDateTime(ViewState["begin"]), Convert.ToDateTime(ViewState["end"]));
        ...
        protected void btn_OutPut_Click(object sender, EventArgs e)
        {
            ...
            model.ProType = int.Parse(this.ddl_ProType.SelectedValue);
            model.Financed = int.Parse(this.ddl_Financed.SelectedValue);
            DataTable dt = jzprojectManageDAL.GetPaged(2000, 1, ref recordCount, model, Convert.ToDateTime(ViewState["begin"]), Convert.ToDateTime(ViewState["end"]));
            if (dt == null || dt.Rows.Count == 0)
            {
                ShowMessage("暂无数据导出！");
                return;
        ...
        protected void lbtn_SB_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                #region 附件上传
                JZProjectManageEntity bt = jzprojectManageDAL.GetObjByID(id);
                if (bt != null)
                {
                    if (bt.PCFile != null)
                    {
