...
{
    public partial class PurchaseImportDetail : PageBase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (PID != "")
                {
                    ...
                    try
                    {
                        GK.GKICMP.Entities.PurchaseEntity model = purchaseDAL.GetObjByID(PID);
                        this.ltl_Project.Text = model.PTitle;
                    }
                    catch (Exception ex)
                    {
