...
{
    public partial class PurchaseAcceptance : PageBase
    {
        ...
        private void DataBindList()
        {
            ...
            //flag区分教装与采购，默认1 位教装
            DataTable dt = project_CheckDAL.GetPaged(Pager.PageSize, Pager.CurrentPageIndex, ref recordCount, model,2);
        ...
        protected void btn_OutPut_Click(object sender, EventArgs e)
        {
            ...
            model.PID = (string)ViewState["GName"];
            //flag区分教装与采购，默认1 位教装
            DataTable dt = project_CheckDAL.GetPaged(9999999, 1, ref recordCount, model,2);
            if (dt == null || dt.Rows.Count == 0)
            {
                ShowMessage("暂无数据导出！");
                return;
        ...
        protected void lbtn_SB_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Project_CheckEntity bt = project_CheckDAL.GetObjByID(id,2);
                if (bt != null)
                {
                    if (bt.PCFile != null)
                    {
                ...
                {
                    Project_CheckEntity p = project_CheckDAL.GetObjByID(id.Split(',')[i],2);
        ...
        protected void lbtn_MoreSB_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                for (int i = 0; i < id.Split(',').Length; i++)
                {
                    Project_CheckEntity bt = project_CheckDAL.GetObjByID(id.Split(',')[i],2);
                    if (bt != null)
                    {
                        if (bt.PCFile != null)
                        {
