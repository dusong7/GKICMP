...
{
    public partial class PurchaseAcceptanceEdit : PageBase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (PCID != "")
                {
                    DataTable dt = purchaseDAL.GetList((int)CommonEnum.Deleted.未删除);
                    CommonFunction.DDlTypeBind(this.ddl_ProName, dt, "PID", "PTitle", "-2");
        ...
        private void InfoBind(int i)
        {
            ...
            if (model != null)
            {
                this.ddl_ProName.SelectedValue = Convert.ToString(model.PID);
