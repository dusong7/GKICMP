<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="PurchaseListMine.aspx.cs" Inherits="GKICMP.purchase.PurchaseListMine" %>
        ...
        <div class="positionc">
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
                <tr>
                    ...
                    <td class="positiona"><a>首页</a><span>></span><asp:Label ID="lbl_ParentMenu" runat="server" Text="采购管理"></asp:Label><span>></span><asp:Label ID="lbl_Menuname" runat="server" Text="我的申请"></asp:Label>
                    </td>
                </tr>
            </table>
        </div>
                            ...
                            <tr>
                                <td style="width: 5px">
                                    <label class="wxz" id='ck_<%#Eval("PID")%>l'>
                                        <input type="checkbox" name="checkbox" onclick="setid(this.id)" value='<%#Eval("PID")%>' <%#int.Parse(Eval("PState").ToString())==(int)GK.GKICMP.Common.CommonEnum.AduitState.通过?Eval("IsReport").ToString()=="0"?"disabled":"disabled":"" %> id='ck_<%#Eval("PID") %>' /></label>
                                </td>
                             ...
                             <td><%#Eval("PEstimate")%></td>
                              ...
                              <%--  <td ><%#Eval("PSDate","{0:yyyy-MM-dd HH:mm}")%></td>--%>
                                ...
                                <td>
                                    ...
                                    <asp:LinkButton ID="lbtn_Report" runat="server" CssClass="listbtn btnreportncolor" ToolTip="上报" 
                                        Visible='<%#int.Parse(Eval("PState").ToString())==(int)GK.GKICMP.Common.CommonEnum.AduitState.通过?Eval("IsReport").ToString()=="1"?int.Parse(Eval("PlState").ToString()==""?"1":Eval("PlState").ToString())==(int)GK.GKICMP.Common.CommonEnum.AduitState.驳回?true:false :true:false%>' CommandArgument='<%#Eval("PID") %>' OnClick="lbtn_SB_Click">上报</asp:LinkButton>
                                </td>
                            </tr>
                        </ItemTemplate>
                    </asp:Repeater>
