<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="VehicleAduitList.aspx.cs" Inherits="GKICMP.vehicle.VehicleAduitList" %>
    ...
    <script type="text/javascript">
        $(function () {
            $('#btn_Audit').click(function () {
                var id = $val("hf_CheckIDS");
                if (checkselectones(id) == false) {
                    alert("系统提示：请至少选择一条信息！");
                    return false;
                } else {
                    //box的ID，Url,data传值，宽，高,Type(-1为添加，0为修改,其他为查看)
                    return openbox('A_id', 'VehicleAduitEdit.aspx', 'id=' + id, 800, 400, 34);
                }
            });
        });
        function viewinfo(e) {
        ...
        <div class="positionc">
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
                <tr>
                    ...
                    <td class="positiona"><a>首页</a><span>></span><asp:Label ID="lbl_ParentMenu" runat="server" Text=""></asp:Label><span>></span><asp:Label ID="lbl_Menuname" runat="server" Text="出车审核"></asp:Label>
                    </td>
                </tr>
            </table>
        </div>
                            ...
                            <tr>
                                <td style="width: 5px">
                                    <label class="wxz" id='ck_<%#Eval("ApplyID")%>l'>
                                        <input type="checkbox" name="checkbox" onclick="setid(this.id)" value='<%#Eval("ApplyID") %>' id='ck_<%#Eval("ApplyID") %>' <%#Eval("VState").ToString()=="0"?"":"disabled" %> /></label>
                                </td>
