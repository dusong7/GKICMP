...
{
    public partial class PersonLog : PageBase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (model != null)
                {
                    ...
                    //DepartmentEntity cmodel = departDAL.GetObj(model.DepID);
                  //  this.ltl_ClassName.Text = model.DepName;
        ...
        private void DataBindList()
        {
            ...
            if (UID != UserID)//判断是否当前登录学生日志
            {
                model = new SpaceLogEntity(UID, (int)CommonEnum.IsorNot.是, -1,-1);
            }
            else
            {
                model = new SpaceLogEntity(UserID, -2, -1,-1);
            }
            model.SFlag = 1;
        ...
        {
            Response.Redirect("PersonPhotos.aspx?id=" + this.hf_UID.Value);
        }
        #endregion
