...
//==判断xls文件
function judgemp(file, node) {
    if (file) {
        var last = file.substring(file.lastIndexOf(".") + 1);
        if (last != "mp4") {
            alert("上传类型错误，只支持.mp4文件！");
            node.outerHTML = node.outerHTML;
        }
    }
}
function judgeExcel(file, node) {
    if (file) {
        var last = file.substring(file.lastIndexOf(".") + 1);
        if (last != "xls" && last != "xlsx") {
            alert("上传类型错误，只支持.xls、.xlsx文件！");
            node.outerHTML = node.outerHTML;
        }
    }
}
...
function judgemp3(file, node) {
    if (file) {
        var suffix = file.split(".");
        var num = suffix.length - 1;
        var name = suffix[num].toLowerCase();
        if (name != "mp3") {
            alert("上传类型错误，暂只支持.mp3的音乐格式！");
            node.outerHTML = node.outerHTML;
        }
    }
}
