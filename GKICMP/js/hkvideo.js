...
$(function () {
    ...
    if (-1 == WebVideoCtrl.I_CheckPluginInstall()) {
        alert("您还未安装过插件，双击WebComponents.exe安装！(该控件只能在32位浏览器下运行，请使用32位IE或360浏览器)");
        window.location.href = "WebComponents.exe";
        return;
    }
...
// 登录
function clickLogin(szIP, szPort, szUsername, szPassword, EquipName) {
    ...
    var iRet = WebVideoCtrl.I_Login(szIP, 1, szPort, szUsername, szPassword, {
        ...
        error: function () {
            //showOPInfo(EquipName + " 登录失败！");
        }
    });

    if (-1 == iRet) {
        //showOPInfo(EquipName + " 已登录过！");
    }
}
    ...
    WebVideoCtrl.I_GetDigitalChannelInfo(szIP, {
        ...
        success: function (xmlDoc) {
            ...
            $.each(oChannels, function (i) {
                ...
                //可以在这里处理一下ip值让其存在相应的名称         
                $.getJSON("../ashx/Video.ashx?method=GetName&IP=" + ip + "&type=digital", function (data) {
