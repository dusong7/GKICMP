...
{
    public partial class SeriesWageManage : PageBase
    {
        ...
        protected void btn_Delete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (result > 0)
                {
                    sysLogDAL.Edit(new SysLogEntity((int)CommonEnum.LogType.操作日志_删除, "删除在编工资信息", UserID));
                    ShowMessage("删除成功");
                }
                else
                {
        ...
        protected void btn_OutPut_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    str.Append("<table border='1' cellpadding='0' cellspaccing='0'><tr><th  rowspan='2'>年份</th><th  rowspan='2'>月份</th><th  rowspan='2'>姓名</th><th colspan='9'>应发工资</th><th colspan='8'>扣除款项</th><th rowspan='2'>实发工资</th><th rowspan='2'>备注</th></tr><tr><th>岗位工资</th><th>薪级工资</th><th>教龄津贴</th><th>教护10%</th><th>基础性绩效工资</th><th>奖励性绩效工资</th><th>提租补贴</th><th>20%工资</th><th>小计</th><th>公积金12%</th><th>失业保险0.5%</th><th>医保费2%</th><th>养老保险12%</th><th>工会费0.5%</th><th>考核工资</th><th>个人所得税</th><th>小计</th></tr>");
                    foreach (DataRow row in dt.Rows)
                    {
                        str.Append("<tr>");
                        str.AppendFormat("<td>{0}</td>", Convert.ToInt32(row["WYear"].ToString()));
                        str.AppendFormat("<td>{0}</td>", Convert.ToInt32(row["WMonth"].ToString()));
                        ...
                        str.AppendFormat("<td  style='vnd.ms-excel.numberformat:0.00'>{0}</td>", Convert.ToDecimal(row["TeachNursing"].ToString()));
                        str.AppendFormat("<td  style='vnd.ms-excel.numberformat:0.00'>{0}</td>", Convert.ToDecimal(row["BasicPay"].ToString()));
                        str.AppendFormat("<td  style='vnd.ms-excel.numberformat:0.00'>{0}</td>", Convert.ToDecimal(row["Rewarding"].ToString()));
                        str.AppendFormat("<td  style='vnd.ms-excel.numberformat:0.00'>{0}</td>", Convert.ToDecimal(row["RentalFee"].ToString()));
                        ...
                        str.AppendFormat("<td  style='vnd.ms-excel.numberformat:0.00'>{0}</td>", Convert.ToDecimal(row["AssessWage"].ToString()));
                        str.AppendFormat("<td  style='vnd.ms-excel.numberformat:0.00'>{0}</td>", Convert.ToDecimal(row["Income"].ToString()));
                        ...
                        str.AppendFormat("<td>{0}</td>", row["WDesc"].ToString());
                        str.Append("</tr>");
                    }
                    str.Append("</table>");
                    CommonFunction.ExportExcel("在编工资", str.ToString());
                    sysLogDAL.Edit(new SysLogEntity((int)CommonEnum.LogType.操作日志_导出, "导出在编工资信息", UserID));               
                }
                else
                {
                    ShowMessage("暂无数据导出");
            ...
            {
                sysLogDAL.Edit(new SysLogEntity((int)CommonEnum.LogType.系统日志, "导出在编工资信息", UserID));
                ShowMessage(ex.Message);
            }
        }
        #endregion
