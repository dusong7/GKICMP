...
{
    public partial class ElectiverStuList : PageBase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                framemain.Attributes["src"] = "ElectiverStuManage.aspx?id=" + did + "&deep=1";
            }
        }
        #endregion
        ...
        private void ChildBind(TreeNode treenode, int gid, int j)
        {
            ...
            if (dv.Rows.Count > 0)
            {
                for (int i = 0; i < dv.Rows.Count; i++)
                {
                    if (j == 0 && i == 0)
                    {
                        ViewState["DID"] = dv.Rows[i]["DID"].ToString();
                    }
                    DataRow row = dv.Rows[i];
        ...
        protected void tv_Meun_SelectedNodeChanged(object sender, EventArgs e)
        {
            ...
            framemain.Attributes["src"] = "ElectiverStuManage.aspx?id=" + svalue + "&deep=" + deep;
        }
        #endregion
    }
}