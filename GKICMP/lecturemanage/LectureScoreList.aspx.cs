...
{
    public partial class LectureScoreList : PageBase
    {
        ...
        {
            int recordCount = -1;
            DataTable dt = lecDAL.GetPaged(Pager.PageSize, Pager.CurrentPageIndex, ref recordCount, ViewState["ClassName"].ToString(), ViewState["TeacherName"].ToString(), Convert.ToDateTime(ViewState["BeginDate"].ToString()), Convert.ToDateTime(ViewState["EndDate"].ToString()), (int)CommonEnum.Deleted.未删除, UserID, 2);
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        protected void lbtn_Score_Click(object sender, EventArgs e)
        {
            LinkButton lbtn = (LinkButton)sender;
            string id = lbtn.CommandArgument;
            Response.Write("<script language=javascript>window.open('LectureScoreEdit.aspx?id=" + id + "', '_self')</script>");
        }
        #endregion
        ...
        protected void lbtn_Detail_Click(object sender, EventArgs e)
        {
            LinkButton lbtn = (LinkButton)sender;
            string id = lbtn.CommandArgument;
            Response.Write("<script language=javascript>window.open('LectureScoreDetail.aspx?id=" + id + "', '_self')</script>");
        } 
        #endregion
    }
}