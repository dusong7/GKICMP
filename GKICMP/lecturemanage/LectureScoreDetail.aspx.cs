...
{
    public partial class LectureScoreDetail : PageBase
    {
 ...
 font-size: 14px;' onclick='jQuery(&#39;#example-advanced&#39;).treetable(&#39;collapseAll&#39;); return false;'>&nbsp;全部收起&nbsp;</a>");
            ...
            if (dtFirst != null && dtFirst.Rows.Count > 0)
            {
                for (int i = 0; i < dtFirst.Rows.Count; i++)
                {
                    ...
                    //sb.Append("<td align='center'>" + scorea.ToString() + "</td>");
        ...
        {
            StringBuilder sb = new StringBuilder("");
            DataTable dv = standardDAL.GetScoreList((int)CommonEnum.Deleted.未删除, parentid, UserID, LID.ToString(), 1);
            if (dv != null && dv.Rows.Count > 0)
            {
                for (int i = 0; i < dv.Rows.Count; i++)
                {
                    ...
                    sb.Append("<td align='center'>" + dv.Rows[i]["Score"].ToString() + "</td>");
                    sb.Append("</tr>");
                    string str = ChildBind(Convert.ToInt32(dv.Rows[i]["LSID"].ToString())).ToString();
                    sb.Append(str.ToString());
                    //scorea += Convert.ToInt32(dv.Rows[i]["Score"].ToString());
                }
            }
            return sb.ToString();
        }
